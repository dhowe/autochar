  ...
  pickNextTarget() {
    ...
    if (useTriggers && !this.memory.contains('trigger')) {
      ...
      for (let i = startIdx; i < opts.length + startIdx; i++) {
        ...
        if (WORD_TRIGGERS.includes(cand)) {
    ...
    if (this.steps === 3) {
      console.log("FORCE TRIGGER: 教徒");
      result = this.util.getWord("教徒");
      triggered = true;
    }
  ...
  doNextEdit() {
    ...
    if (this.action == REPLACE_STROKE) {
      if (this.word.nextStroke(this.targetCharIdx, this.targetPartIdx)) {
        ...
        this.pickNextTarget();
        this.onActionCallback(finished, this.target, this.nextIsTrigger); // pass the next word
        this.readyToSendNext = true;
        //this.target = null;
        //this.doNext();
      }
